...
/// calculates the time overflow and the time overflow delta.
///
/// (time_overflow, time_overflow_delta)
pub fn calculate_time_overflow(time_difference: Time, total_day_time: Time) -> (Time, Time) {
    ...
    let time_overflow_delta = new_overflow - old_overflow;
    (new_overflow, time_overflow_delta)
}

/// Calculates the capacity overflow and the capacity overflow delta.
///
/// (capacity_overflow, capacity_overflow_delta)
///
...
pub fn calculate_capacity_overflow(capacity_difference: i32, route_capacity: i32) -> (i32, i32) {
    ...
    let overflow_delta = new_overflow - old_overflow;

    (new_overflow, overflow_delta)
